...
class RSSController {
  constructor ($scope,$log,$http,moment,mySocket,$sce,$window,$cookies,toastr) {
    'ngInject';
  ...
  init(){
    ...
    this.mySocket.on("connect_error",() => {
      ...
      this.toastr.error("Connection goes down! Check your internet connection or forgive me for several minutes :)");
    })
    this.mySocket.on("reconnect",() => {
      this.toastr.success("reconnect!");
    })
      ...
      switch (response.type) {
        case "xframe":
          ...
          if(!this.status.loadings.linkIsFine){
            this.status.viewUrl = "";
            this.toastr.error(`<a style="color:white;" href='${response.link}' target='_blacnk'>Can't Load. Click Here To View</a>`);
            this.randomPic();
          }
  ...
  deleteFeed(feedUrl,$event){
    ...
    if (feedUrl == this.status.expandUrl) {
      // if delete the current expanded feed
      this.status.expandUrl = "-1"
    }
  ...
  doFeed(action,data){
    ...
    // logout
    if(data.content == "logout"){
      this.$cookie.remove("user");
      this.status.userActive = false;
      return
    }
    var request = {}
    if(action == "add"){
