...
class Board{
  ...
  randomBoard(w,h){
    ...
    var end = this.putWordInBoard(board,"HI",0,0,7);
    this.putWordInBoard(board,"TAO",0,end,0);
    var n = this.putWordInBoard(board,"IT",1,0,7);
    this.putWordInBoard(board,"IS",1,n,3);
    ...
    this.putWordInBoard(board,"O'CLOCK",board.length-1,2,0);
    return board
  }
...
class WidgetController {
  constructor ($scope,$log,$element,$http,moment,$attrs,mySocket,$interval) {
    ...
    if($attrs.name == "clock"){
      ...
      this.lastMinute = "";
      this.renderBoard();
      $interval(this.renderBoard.bind(this),10*1000);
    }
  }
  renderBoard(){
    if (this.lastMinute == (new Date()).getMinutes()) {
      return
    }
