...
class TodoController {
  constructor ($scope,$log,$http,moment,mySocket) {
    ...
    this.$scope.$on('outer', function() {
      _this.$log.info("TODO part Outer Render Finished at:"+moment().second())
    })
  ...
  addTodo(todo){
    ...
    this.originTodoData.data[date].expandItem = true
  ...
  formatData(flag){
    ...
    Object.keys(this.originTodoData.data).sort().reverse().forEach( (key) =>{
    ...
    // need force digest for fist time loading data
    if(this.firstLoad){
      this.$scope.$digest()
    }
  ...
  readFromDisk(){
    ...
    this.$log.info("getting todo data..."+this.moment().second())
    this.mySocket.on('todoData', (data) => {
      _this.$log.info("got todo data..."+this.moment().second())
      _this.gotData = true
      _this.originTodoData = data
      _this.formatData()
    });
