...
class MalarkeyController {
  constructor ($log, todoList) {
    'ngInject';

    this.$log = $log;
    this.contributors = [];

    this.activate(todoList);
  }

  activate(todoList) {
    return this.getContributors(todoList).then(() => {
      this.$log.info('Activated Contributors View');
    });
  }

  getContributors(todoList) {
    return todoList.getContributors(10).then((data) => {
